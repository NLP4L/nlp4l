...
package org.nlp4l.lucene.stats
...
//    Try(path.deleteRecursively(continueOnFailure = false))
  ...
  test("generate a TF term vector without Lucene Index") {
    // idf = n
    val vector1: Seq[Double] = TFIDF.tfIdfVector(Map( "A"->3, "B"->2  ), Set("A", "B"), "n", 0.0, "n", Map.empty, 100, -1, Map( "A"->10, "B"->10  ))
    assertResult(Vector(3.0,2.0))(vector1)
    // idf = t
    val vector2: Seq[Double] = TFIDF.tfIdfVector(Map( "A"->3), Set("A"), "n", 0.0, "t", Map.empty, 100, -1, Map( "A"->10 ))
    assertResult(Vector(3 * math.log(100 / 10.0)))(vector2)
    // idf = T
    val vector3: Seq[Double] = TFIDF.tfIdfVector(Map( "A"->3), Set("A"), "n", 0.0, "T", Map.empty, 100, -1, Map( "A"->10 ))
    assertResult(Vector(3 * math.log(101 / 11.0)))(vector3)
    // idf = p
    val vector4: Seq[Double] = TFIDF.tfIdfVector(Map( "A"->3), Set("A"), "n", 0.0, "p", Map.empty, 100, -1, Map( "A"->10 ))
    assertResult(Vector(3 * math.max(0, math.log((100 - 10.0) / 10.0))))(vector4)
    // idf = P
    val vector5: Seq[Double] = TFIDF.tfIdfVector(Map( "A"->3), Set("A"), "n", 0.0, "P", Map.empty, 100, -1, Map( "A"->10 ))
    assertResult(Vector(3 * math.max(0, math.log((101 - 11.0) / 11.0))))(vector5)
  }
