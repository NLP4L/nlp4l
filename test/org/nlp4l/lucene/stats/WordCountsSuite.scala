...
//    Try(path.deleteRecursively(continueOnFailure = false))
  ...
  test("counts words from a plain text") {
    val words = Set("london", "gold", "down")
    val counts = WordCounts.countWords("London Bridge is falling down, Falling down, Falling down. London Bridge is falling down, My fair lady.", words, contentAnalyzer)
    assertResult(2)(counts.getOrElse("london", 0))
    assertResult(0)(counts.getOrElse("gold", 0))
    assertResult(4)(counts.getOrElse("down", 0))
  }
}
