 ...
 *
  ...
  def countWords(text: String, words: Set[String], analyzer: Analyzer): Map[String, Long] = {
    val termsMap = mutable.Map.empty[String, Long]
    analyzer.tokens(text).filter(token => if (words.isEmpty) true else words.contains(token.get("term").get))
      .foreach(token => termsMap += (token.get("term").get -> (termsMap.getOrElse(token.get("term").get, 0L) + 1L)))
    termsMap.toMap;
  }
