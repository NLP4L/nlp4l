...
import org.nlp4l.lucene.{RawReader, IReader}
...
object TFIDF {
   ...
   */
  private def idf(v: Long, mode: String, numDocs: Option[Int] = None): Double = {
    mode match {
      // no
      case "n" => 1
      // idf
      ...
      // idf (document count plus one)
      case "T" => numDocs match {
        case Some(value) => math.log((value+1) / (v+1).toDouble)
        case _ => throw new IllegalThreadStateException("numDocs parameter is required.")
      }
      // prob idf
      ...
      // prob idf (document count plus one)
      case "P" => numDocs match {
        case Some(value) => math.max(0, math.log(((value+1) - (v+1)) / (v+1).toDouble))
        case _ => throw new IllegalThreadStateException("numDocs parameter is required.")
      }
      // unknown
      case _ => throw new IllegalArgumentException("Unknown notation: " + mode)
  ...
  // tfIdfVector without lucene index
  def tfIdfVector(countMap: Map[String, Long], words: Set[String], tfMode: String, smthTerm: Double, idfMode: String, termBoosts: Map[String, Double], numDocs: Int, maxTF: Long, dfMap: Map[String, Long]): Seq[Double] = {
    val aveTF = if (tfMode == "L") Some(Stats.average(countMap.values)) else None
    val features = words.toSeq.sorted
    val vector = features.map(w => if (countMap.contains(w)) tf(countMap(w), tfMode, Some(maxTF), smthTerm, aveTF) * termBoosts.getOrElse(w, 1.0) * idf(dfMap(w), idfMode, Some(numDocs)) else 0.0).toVector
    vector
  }
}
